...
export function calculateAuditScore(
  ...
  const completionRate =
    totalCriteria > 0 ? Math.round((totalCompleted / totalCriteria) * 1000) / 10 : 0;
...
function scoreSingleCategory(
  cat: CategoryInput,
  responseMap: Map<string, ResponseInput>,
): ScoredCategory {
  let catScoreSum = 0;
  let catScored = 0;
  let catPassed = 0;
  ...
  for (const crit of cat.criteria) {
    ...
    if (critType === 'SCORED') {
      ...
      if (resp.passed === true) { catPassed++; }
      else if (resp.passed === false) { catFailed++; }
...
function isResponseCompleted(critType: string, resp: ResponseInput): boolean {
  switch (critType) {
    case 'SCORED':
