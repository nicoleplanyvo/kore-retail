...
vmComplianceRouter.post('/areas/:id/pdf', pdfUpload.single('pdf'), async (req, res) => {
  try {
    ...
    if (!existing) return res.status(404).json({ error: 'Bereich nicht gefunden.' });
    if (!req.file) return res.status(400).json({ error: 'PDF-Datei ist erforderlich.' });
...
vmComplianceRouter.post('/checks', photoUpload.single('photo'), async (req, res) => {
  try {
    ...
    if (parsed.data.areaId) {
      ...
      if (!area) return res.status(404).json({ error: 'Bereich nicht gefunden.' });
    }
