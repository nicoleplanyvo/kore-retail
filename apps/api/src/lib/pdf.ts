...
export function createPdfDocument(title = 'KORE Report'): InstanceType<typeof PDFDocument> {
...
export function addHeader(doc: InstanceType<typeof PDFDocument>, title: string, subtitle?: string) {
...
export function addSectionTitle(doc: InstanceType<typeof PDFDocument>, title: string) {
  const y = doc.y;
...
export function addKeyValue(doc: InstanceType<typeof PDFDocument>, label: string, value: string) {
  const y = doc.y;
...
export function addFooter(doc: InstanceType<typeof PDFDocument>) {
  ...
  for (let i = pages.start; i < pages.start + pages.count; i++) {
    ...
    doc.fontSize(8).fillColor(COLORS.mid);
    doc.text(
      `KORE Retail Platform  --  Seite ${i + 1} von ${pages.count}  --  ${new Date().toLocaleDateString('de-DE')}`,
      50,
      780,
