...
export const checklistenSessionsRouter = Router();
...
checklistenSessionsRouter.post('/', requireMinRole('store_manager'), async (req, res) => {
    try {
...
checklistenSessionsRouter.post('/:id/complete', requireMinRole('store_manager'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const sessionId = req.params['id'];
        const session = await prisma.checklistSession.findUnique({
        ...
        const totalItems = session.template.sections.reduce((sum, sec) => sum + sec.items.length, 0);
...
checklistenSessionsRouter.delete('/:id', requireMinRole('store_manager'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const sessionId = req.params['id'];
