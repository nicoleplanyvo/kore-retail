...
wellbeingRouter.get('/checkins', async (req, res) => {
    try {
        ...
        if (req.query.to)
            where['date'] = { ...(where['date'] || {}), lte: new Date(req.query.to + 'T23:59:59') };
...
wellbeingRouter.post('/checkins', async (req, res) => {
    try {
        ...
        if (parsed.data.moodScore < 2) {
            console.warn(`[WELLBEING ALERT] Kritischer Stimmungswert (${parsed.data.moodScore}) in Tenant ${tenantId}` +
    ...
    try {
        ...
        for (const c of checkIns) {
            ...
            if (!dateMap.has(dateStr))
                dateMap.set(dateStr, { moods: [], count: 0 });
