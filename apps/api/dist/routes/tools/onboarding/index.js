...
onboardingRouter.get('/journeys', async (req, res) => {
    try {
        ...
        const enriched = await Promise.all(data.map(async (j) => {
...
onboardingRouter.get('/dashboard', async (req, res) => {
    try {
        ...
        const avgProgress = activeJourneys.length > 0
            ? Math.round(activeJourneys.reduce((sum, j) => {
        ...
        const avgCompletionDays = completedJourneys.length > 0
            ? Math.round(completedJourneys.reduce((sum, j) => {
