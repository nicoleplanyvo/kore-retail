...
export const caReportsRouter = Router();
...
caReportsRouter.get('/dashboard', async (req, res) => {
    try {
        ...
        // KPIs parallel laden
        const [totalAudits, completedAudits, todaysSessions, overdueCount, recentCompleted,] = await Promise.all([
        ...
        const avgCompletionRate = recentCompleted.length > 0
            ? Math.round((recentCompleted.reduce((s, c) => s + c.completionRate, 0) /
...
// ── Helpers ──────────────────────────────────────────
function buildBaseWhere(tenantId, toolStoreIds, storeId) {
