...
export const caTemplatesRouter = Router();
// ── Zod Schemas ──────────────────────────────────────
...
// ── POST /templates ──────────────────────────────────
caTemplatesRouter.post('/', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
...
// ── PUT /templates/:id ───────────────────────────────
caTemplatesRouter.put('/:id', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
...
// ── DELETE /templates/:id ────────────────────────────
caTemplatesRouter.delete('/:id', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const templateId = req.params['id'];
        const existing = await prisma.auditTemplate.findUnique({
...
caTemplatesRouter.post('/:id/clone', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const userId = req.user.sub;
