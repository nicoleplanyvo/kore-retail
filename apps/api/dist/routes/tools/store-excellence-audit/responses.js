...
export const seaResponsesRouter = Router();
const upload = createToolUpload('store-excellence-audit');
// PUT /sessions/:sessionId/responses/:criterionId — Upsert Bewertung
seaResponsesRouter.put('/sessions/:sessionId/responses/:criterionId', requireRole('tenant_admin', 'store_manager', 'kore_admin'), async (req, res) => {
    try {
...
seaResponsesRouter.post('/sessions/:sessionId/responses/:criterionId/photo', requireRole('tenant_admin', 'store_manager', 'kore_admin'), upload.single('photo'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const sessionId = req.params['sessionId'];
        const criterionId = req.params['criterionId'];
        ...
        const relativePath = getRelativePath('store-excellence-audit', tenantId, sessionId, req.file.filename);
...
seaResponsesRouter.delete('/sessions/:sessionId/responses/:criterionId/photo', requireRole('tenant_admin', 'store_manager', 'kore_admin'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const sessionId = req.params['sessionId'];
        const criterionId = req.params['criterionId'];
