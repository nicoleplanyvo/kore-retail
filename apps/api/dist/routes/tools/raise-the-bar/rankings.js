...
// ---------- Scoring helpers ----------
function normalizeValue(value, target, higherIsBetter, allValues) {
...
function buildRankings(stores, indicators, entries) {
    // Group entries by indicator to get all values for normalization
    const entriesByIndicator = new Map();
    const allValuesByIndicator = new Map();
    ...
    const storeRankings = stores.map((store) => {
        const indicatorScores = indicators.map((ind) => {
        ...
        const totalScore = indicatorScores.reduce((sum, is) => sum + is.score * is.weight, 0);
        return {
            storeId: store.id,
...
rankingsRouter.get('/comparison', async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const storeIds = (req.query['storeIds'] ?? '').split(',').filter(Boolean);
        const period = req.query['period'] ?? getCurrentPeriod();
...
rankingsRouter.post('/recalculate', async (req, res) => {
    try {
        ...
        const rankings = buildRankings(filteredStores, indicators, entries);
        // Upsert snapshots
        const upserts = rankings.map((r) => prisma.rtbSnapshot.upsert({
