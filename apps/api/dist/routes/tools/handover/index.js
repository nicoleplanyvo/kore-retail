...
handoverRouter.get('/dashboard', async (req, res) => {
    try {
        ...
        if (req.query.from || req.query.to) {
            where['shiftDate'] = {};
            if (req.query.from)
                where['shiftDate']['gte'] = req.query.from;
            if (req.query.to)
                where['shiftDate']['lte'] = req.query.to;
        }
        ...
        const complianceRate = total > 0 ? Math.round((acknowledged / total) * 100) : 0;
        const fastAckRate = acknowledgedHandovers.length > 0 ? Math.round((within2h / acknowledgedHandovers.length) * 100) : 0;
        // Unacknowledged list
