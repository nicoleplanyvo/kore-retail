...
import { stockCalloutCreateSchema, stockCalloutUpdateSchema, stockCalloutOfferSchema, } from '../../../shared/validators.js';
...
stockCalloutsRouter.get('/callouts', async (req, res) => {
    try {
        ...
        if (req.query.sku)
            where['sku'] = { contains: req.query.sku };
...
stockCalloutsRouter.put('/callouts/:id/offer', async (req, res) => {
    try {
        ...
        if (!callout)
            return res.status(404).json({ error: 'Callout nicht gefunden.' });
        if (callout.status !== 'OPEN')
            return res.status(400).json({ error: 'Callout ist nicht mehr offen.' });
...
stockCalloutsRouter.put('/callouts/:id/transfer', async (req, res) => {
    try {
        ...
        if (!callout)
            return res.status(404).json({ error: 'Callout nicht gefunden.' });
        if (callout.status !== 'OFFERED')
            return res.status(400).json({ error: 'Callout hat keinen offenen Angebotstatus.' });
        const updated = await prisma.stockCallout.update({
            where: { id: req.params['id'] },
...
stockCalloutsRouter.get('/dashboard', async (req, res) => {
    try {
        ...
        callouts.forEach(c => {
            const existing = skuMap.get(c.sku) || { sku: c.sku, productName: c.productName, count: 0, totalRequested: 0, stores: new Set() };
            existing.count++;
        ...
        callouts.forEach(c => {
            ...
            existing.total++;
            if (c.status === 'OPEN' || c.status === 'OFFERED')
                existing.open++;
