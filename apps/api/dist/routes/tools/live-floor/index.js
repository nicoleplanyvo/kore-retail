...
import { floorZoneCreateSchema, floorZoneUpdateSchema, floorPositionCreateSchema, floorPositionUpdateSchema, floorFrequencyUpdateSchema, floorBulkAssignSchema, } from '../../../shared/validators.js';
...
liveFloorRouter.get('/recommendations', async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const toolStoreIds = req.toolStoreIds;
        const zoneWhere = { tenantId, isActive: true };
        if (req.query.storeId)
            zoneWhere['storeId'] = req.query.storeId;
        else if (toolStoreIds !== 'all')
            zoneWhere['storeId'] = { in: toolStoreIds };
        const zones = await prisma.floorZone.findMany({
            where: zoneWhere,
            include: {
                store: { select: { id: true, name: true } },
        ...
        const recommendations = zones
            ...
            let status = 'OK';
            if (staffCount < z.minStaff || deficit >= 2)
                status = 'UNTERBESETZT';
            else if (deficit >= 1)
                status = 'ACHTUNG';
            return {
...
liveFloorRouter.get('/dashboard', async (req, res) => {
    try {
        ...
        const logWhere = { tenantId, date: { gte: cutoffDate } };
        if (storeId)
            logWhere['storeId'] = storeId;
        else if (toolStoreIds !== 'all')
            logWhere['storeId'] = { in: toolStoreIds };
        ...
        for (const l of logs) {
            if (!hourlyMap[l.hour])
                hourlyMap[l.hour] = { customers: 0, staff: 0, count: 0 };
        ...
        for (const l of logs) {
            if (!dailyMap[l.date])
                dailyMap[l.date] = { customers: 0, staff: 0, count: 0 };
