...
if (!fs.existsSync(guidelinePhotoDir))
    fs.mkdirSync(guidelinePhotoDir, { recursive: true });
const upload = multer({
...
if (!fs.existsSync(guidelinePdfDir))
    fs.mkdirSync(guidelinePdfDir, { recursive: true });
...
if (!fs.existsSync(areaPdfDir))
    fs.mkdirSync(areaPdfDir, { recursive: true });
...
vmComplianceRouter.post('/areas/:id/pdf', areaPdfUpload.single('pdf'), async (req, res) => {
    try {
        ...
        const areaId = req.params['id'];
        if (!req.file)
            return res.status(400).json({ error: 'PDF ist erforderlich.' });
        const existing = await prisma.vmArea.findFirst({ where: { id: areaId, tenantId } });
        if (!existing)
            return res.status(404).json({ error: 'Bereich nicht gefunden.' });
        // Alte PDF loeschen
