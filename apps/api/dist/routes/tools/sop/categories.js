...
sopCategoriesRouter.post('/', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
...
sopCategoriesRouter.put('/:id', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
        const result = sopCategoryCreateSchema.safeParse(req.body);
        if (!result.success) {
            res.status(400).json({
...
sopCategoriesRouter.delete('/:id', requireRole('tenant_admin', 'kore_admin'), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const categoryId = req.params['id'];
        const existing = await prisma.sopCategory.findUnique({
