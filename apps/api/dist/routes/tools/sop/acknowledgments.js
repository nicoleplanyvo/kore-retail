...
export const sopAcknowledgmentsRouter = Router();
...
sopAcknowledgmentsRouter.get('/reports/acknowledgment-status', async (req, res) => {
    try {
        ...
        const report = await Promise.all(sops.map(async (sop) => {
            ...
            return {
                ...
                acknowledgedCount,
                totalUsers,
                acknowledgedPercent: totalUsers > 0
...
sopAcknowledgmentsRouter.get('/reports/compliance', async (req, res) => {
    try {
        ...
        const users = visibleUsers.map((user) => {
            const readCount = mandatorySopIds.filter((sopId) => ackSet.has(`${user.id}:${sopId}`)).length;
            return {
                userId: user.id,
                name: user.name,
                email: user.email,
