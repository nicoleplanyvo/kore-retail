...
import { createPdfDocument, addHeader, addSectionTitle, addKeyValue, addTable, addFooter, fmtDe, fmtEur, } from '../../lib/pdf.js';
export const reportingExportRouter = Router();
...
reportingExportRouter.get('/kpi', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="kpi-report-${safePeriod}.pdf"`);
        doc.pipe(res);
...
reportingExportRouter.get('/audit', async (req, res) => {
    try {
        ...
        const periodLabel = dateFrom && dateTo ? `${dateFrom} bis ${dateTo}` : 'Gesamtzeitraum';
        const safePeriod = (dateFrom ?? 'alle').replace(/\//g, '-');
        res.setHeader('Content-Disposition', `attachment; filename="audit-report-${safePeriod}.pdf"`);
        doc.pipe(res);
...
reportingExportRouter.get('/store-overview', async (req, res) => {
    try {
        ...
        addKeyValue(doc, 'Transaktionen', fmtDe(totalTransactions));
        addKeyValue(doc, 'Kundenfrequenz', fmtDe(totalFootfall));
        addKeyValue(doc, 'Conversion-Rate', totalFootfall > 0
...
//# sourceMappingURL=reporting-export.js.map