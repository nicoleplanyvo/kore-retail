...
export function WeekViewPage() {
  ...
  return (
    <div className="p-xl max-w-6xl">
      <Breadcrumb items={[{ label: 'Schichtplanung', href: '/app/tools/shift-planning' }, { label: 'Wochenansicht' }]} />
      <div className="flex items-center gap-md mb-2xl">
        <Link to="/app/tools/shift-planning" className="text-kore-mid hover:text-kore-ink transition-colors"><ArrowLeft size={20} /></Link>
        <h1 className="font-display text-h1 text-kore-ink flex-1">Wochenansicht</h1>
        <div className="flex items-center gap-sm">
          <button onClick={() => setWeekOffset(w => w - 1)} className="p-sm border border-kore-border hover:bg-kore-bg" aria-label="Vorherige Woche"><ChevronLeft size={16} /></button>
          <button onClick={() => setWeekOffset(0)} className="px-md py-sm border border-kore-border text-small hover:bg-kore-bg">Heute</button>
          <button onClick={() => setWeekOffset(w => w + 1)} className="p-sm border border-kore-border hover:bg-kore-bg" aria-label="Nächste Woche"><ChevronRight size={16} /></button>
        </div>
